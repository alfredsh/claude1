...
import { doctorAPI, BACKEND_URL } from '@/lib/api'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Badge } from '@/components/ui/badge'
import { FlaskConical, Pill, Star, Brain, Loader2, ChevronDown, ChevronUp, User, FileHeart, Activity, Scan, Microscope, Wind, Radiation, FileText, ImageIcon, X } from 'lucide-react'
...
export default function PatientDetail() {
  ...
  const { data: patientDocs = [] } = useQuery({ queryKey: ['doctor-patient-docs', id], queryFn: () => doctorAPI.getPatientDocs(id!).then(r => r.data) })
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <FileHeart className="w-5 h-5 text-rose-500" /> Исследования
            {patientDocs.length > 0 && (
              <Badge variant="outline" className="ml-auto text-xs">{patientDocs.length}</Badge>
            )}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {patientDocs.length === 0 ? (
            <p className="text-sm text-slate-400 text-center py-6">Пациент ещё не загрузил исследования</p>
          ) : (
            <div className="space-y-3">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{formatDate(doc.docDate)}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge
                        ...
                        >
                          {doc.status === 'completed' ? '✓ Обработан' : doc.status === 'processing'
                            ? <span className="flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin" />Анализ…</span>
                        ...
                        {doc.aiSummary && (
                          <div className="bg-gradient-to-r from-blue-50 to-teal-50 rounded-xl p-3 border border-blue-100">
                            <div className="flex items-center gap-2 mb-1.5">
                              ...
                              <span className="text-xs font-semibold text-blue-800">ИИ-заключение</span>
                            </div>
                            <p className="text-xs text-slate-700 leading-relaxed whitespace-pre-wrap">{doc.aiSummary}</p>
                          </div>
                        )}
                        {doc.status === 'processing' && !doc.aiSummary && (
                          <div className="flex items-center gap-2 text-xs text-slate-500">
                            <Loader2 className="w-3.5 h-3.5 animate-spin" /> Анализ выполняется…
                          </div>
                        )}
                      </div>
                    )}
